...
class VariationalUNet(nn.Module):
    def __init__(self, height=1028, width=92, latent_dim=128):
        super().__init__()
        ...
        self.fc_mu = nn.Linear(64 * (height // 4) * (width // 4), latent_dim)  # Mean of latent distribution
        self.fc_logvar = nn.Linear(64 * (height // 4) * (width // 4), latent_dim)  # Log variance of latent distribution
        self.fc_decoder = nn.Linear(latent_dim, 64 * (height // 4) * (width // 4))  # Map latent vector back to feature map
    ...
    def forward(self, x, verbose=False):
        ...
        output = F.interpolate(raw_output, size=(self.height, self.width), mode='bilinear', align_corners=False)
