...
class UNetConv12(nn.Module):
    ...
    def forward(self, x, t=0):
        ...
        d4 = e4 + b #self.attn4(e4, b) --- # i should try and replace these with concatenations in the future
        d4 = self.dec4(d4, embeddings)

        # For decoder stage 3
        d4 = F.interpolate(d4, size=e3.shape[2:], mode="bilinear", align_corners=False)
        d3 = e3 + d4 #self.attn3(e3, d4)
        d3 = self.dec3(d3, embeddings)

        # For decoder stage 2
        d3 = F.interpolate(d3, size=e2.shape[2:], mode="bilinear", align_corners=False)
        d2 = e2 + d3 #self.attn2(e2, d3)
        d2 = self.dec2(d2, embeddings)

        # For decoder stage 1
        d2 = F.interpolate(d2, size=e1.shape[2:], mode="bilinear", align_corners=False)
        d1 = e1 + d2 #self.attn1(e1, d2)
